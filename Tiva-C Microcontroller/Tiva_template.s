...
SOMEDELAY             EQU 2700000      ; a faster than 500ms delay given the ~16 MHz clock
...
NextChar
		...
		BNE		ProcessChar	           ; branch and process if we have a character 

      ; If we read the null at end of the string - add the end of string delay and then reset R5 to the start of character string

		MOV		R0, #4		
		BL		DELAY        ; delay 4 extra spaces (7 total) between words  
		B       ResetLUT     ;reset R5 to the start of character string
...
; "falls" into the C flag - then one can BCC (Branch Carry Clear) or BCS (Branch Carry Set) if the set was 0 or 1
		...
		CLZ R2, R1        ;count the leading zero's of R1
        LSL R1, R1, R2     ;shift left with number R2 bits
...
Shift_and_display
		...
		BLCC	LED_OFF	; branch to turn off the LED if it's zero because the Carry flag is clear
		BLCS	LED_ON	; branch to turn on the LED if it's not zero because the Carry flag is set
...
;  init R0 before calling DELAY to set how long to delay
		MOV R0,#1               
		BL	DELAY      ; one unit delay after turning on or off
		BL LED_OFF
		TEQ R1,#0      ; test if R1 is not 0, branch to shift R1 left. 
		BNE Shift_and_display      
...
;  If we're done then branch back to get the next character
        MOV R0,#3        
		BL DELAY       ; 3 unit delay after each character
		B NextChar 	;   Branch to execute next character; This is the end of the main program 
...
CHAR2MORSE	STMFD		R13!,{R2,R14}	; push Link Register (return address) on stack
		;
				SUB R0,R0,#0x41         ;subract #0x41 from R0 to get an index         add code here to convert the ASCII code to an index (subtract 41) and lookup the Morse patter in the Lookup Table
		        ADD R0,R0,R0            ; double the index (1 byte for one character and 1 byte empty)
				LDR R2,=MorseLUT          ; load Morse Lookup Table to R2. 
				LDRH R1,[R2,R0]          ;load binary code (half world) from the MLT to Register 1. 
		;
		   LDMFD		R13!,{R2,R15}	; restore  R2 and link Register to R15 the Program Counter to return
...
LED_ON 	   	;push 		{r3-r4}		; preserve R3 and R4 on the R13 stack
			STMFD   R13!,{R3,R4,R14}         ; store R3, R4 and link register to stack              
			MOV R3,#RED                     ;initiallze R3 
			LDR R4, =GPIO_PORTF_DATA_R       ;  assign port to R4
			STR R3,[R4]                       ;turn on the 
			LDMFD   R13!,{R3,R4,R15}          ;retore R3, R4 and link register to the program counter 
		;... insert your code here
		;pop 		{r3-r4}		; restore R3 and R4 before returning
...
;	OUTPUT:  none
;
LED_OFF	   	STMFD		R13!,{R3,R4,R14}	; push R3, R4 and Link Register (return address) on stack
			MOV R3,#0                         ; initialize R3 as 0
			LDR R4, =GPIO_PORTF_DATA_R        ; store port address to R4     
			STR R3,[R4]                      ; turn off the LED 
...
MultipleDelay		TEQ		R0, #0		; test R0 to see if it's 0 and if so exit - this sets the Zero flag so you can use BEQ, BNE
			;... insert your code here
				BEQ  exitDelay      ;exit if R0 is 0
				LDR R2, =SOMEDELAY         ;add one unit delay(500ms) to R2. 
loop
				SUBS R2,R2,#1          ;substract 1 from R2  ---for unit delay 
				BNE  loop                 ;Branch the loop if R2 is not 0 
				SUB R0,R0,#1             ; subtract 1 from R0       
				B    MultipleDelay        ; for multiple delay until R0 is 0 (exit)
exitDelay		LDMFD		R13!,{R2, R15}     ;restore R2 and link register
...
InputLUT	DCB		"AWJLB", 0	; string consist of initials must be stored, and read, as BYTES
